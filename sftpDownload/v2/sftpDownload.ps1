...
try {
    ...
    $libFolder = Get-VstsInput -Name libFolder
    if ($libFolder) {
        Write-VstsTaskVerbose "Searching for Renci.SshNet.dll in the $libFolder"
        pushd $libFolder
        $fi = ls -Force -Recurse -File Renci.SshNet.dll
        if (!$fi) {
            throw "Renci.SshNet.dll not found in $libFolder"
        }
        popd
    }
    else {
    ...
    if ($fi -is [System.IO.FileInfo]) {
        ...
        $libFolder = $fi.DirectoryName
    }
    popd

    [System.AppDomain]::CurrentDomain.add_AssemblyResolve({
            param($senderObj, $ResolveEventArgs)
    
            # Get the assembly name
            Write-VstsTaskVerbose -Message "Assembly load: requested name: '$($ResolveEventArgs.Name)'"
            $assemblyName = New-Object System.Reflection.AssemblyName($ResolveEventArgs.Name)
    
            # Define the path to the DLL in the lib folder
            $assemblyPath = Join-Path $libFolder "$($assemblyName.Name).dll"
    
            # If the DLL exists in the lib folder, load it
            if (Test-Path $assemblyPath) {
                Write-VstsTaskVerbose -Message "Assembly load: loading from: '$assemblyPath'"
    ...
    if ($appInsights) {
        $appInsights.Context.GlobalProperties["Renci.SshNet"] = [Renci.SshNet.SftpClient].Assembly.GetName().Version.ToString()
    }
    ...
    if ($appInsights -and $Error -and $Error[0].Exception) {
        ...
        while ($lastException) {
            Write-Warning "$($lastException.GetType().FullName)"
            $lastException = $lastException.InnerException
        }
        $lastException = $Error[0].Exception
        while ($lastException) {
            if ($lastException -is [System.Reflection.ReflectionTypeLoadException]) {
                Write-Warning "Error loading types: $($lastException.Message)"
                [System.Reflection.ReflectionTypeLoadException]$typeLoadException = $lastException
                $typeLoadException.LoaderExceptions | ForEach-Object {
                    $appInsights.TrackException($_)
                    Write-Warning $_
                }
            }
            $lastException = $lastException.InnerException
